...
2. **На сервере** откройте PowerShell от имени администратора и выполните:
   ...
   сам: скачает `node.exe` и `caddy.exe`, создаст `C:\weglow-advice`, откроет
   порты, зарегистрирует две задачи планировщика с автозапуском и покажет
   результат самопроверки. Если домен другой:
...
- `selftest` по HTTPS не открывается, а `http://127.0.0.1:8787/advice?selftest=1`
  ...
  смотрите `caddy.log`.
- В панели советника «сервер советника не отвечает» — сервис или Caddy не
  запущены: `Get-ScheduledTask 'WeGlow *' | Get-ScheduledTaskInfo`.
- Порты 80/443 заняты IIS — остановите IIS (`Stop-Service W3SVC`,
  `Set-Service W3SVC -StartupType Disabled`) и запустите установку снова.
